...
int z=0;
while(z<2){
		z=waitpid(pid3,&status,0);
		z++;
}

if(z==-1){
		int mark1=pid3;
		mysigval.sival_int=mark1;
		paused=1;
...
int z=0;
while(z<2){
		z=waitpid(pid,&status,0);
		z++;
}
		if(z==-1){
		int mark2=pid;		
		mysigval.sival_int=mark2;
		paused=1;
		sigqueue(pid3,SIGUSR2,mysigval);
		sleep(4);
		waitpid(pid,&status,0);
}
		int totalSum = b + secondHalfSum;
		fprintf(outputf,"Sum=%d\n",totalSum);
		fprintf(outputf,"Terminated processes=0\n");
		fprintf(outputf,"Live processes=%d",paused);
